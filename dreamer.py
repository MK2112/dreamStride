...
## Our Implementation:
# python dreamer.py --env spot-walk --algo Dreamerv2 --exp spot-webots --train
# python dreamer.py --env spot-walk --algo Dreamerv2 --exp spot-webots --evaluate

## Sanity Check from Documentation:
# python dreamer.py --env walker-walk --algo Dreamerv2 --exp walker-tester --train
...
class Dreamer:
    ...
    def __init__(self, args, obs_shape, action_size, device, restore=False):
        ...
        self.data_buffer = ReplayBuffer(self.args.buffer_size, self.obs_shape, self.action_size, self.args.train_seq_len, self.args.batch_size)
...
def main():
    ...
    # General parameters
    parser.add_argument('--env', type=str, default='spot-walk', help='Control Suite environment')
    parser.add_argument('--algo', type=str, default='Dreamerv2', choices=['Dreamerv1', 'Dreamerv2'], help='Applied algorithm')
    parser.add_argument('--exp', type=str, default='spot-webots', help='Name of experiment for logging')
    parser.add_argument('--train', action='store_true', help='Train the model')
    parser.add_argument('--evaluate', action='store_true', help='Test the model')
    parser.add_argument('--seed', type=int, default=1, help='Seed for random number generator reproductibility')
    parser.add_argument('--gpu', action='store_false', help='GPUs are used if passed')
    ...
    parser.add_argument('--time-limit', type=int, default=2000, help='Time limit')
    ...
    parser.add_argument('--obs-embed-size', type=int, default=1024, help='Observation embedding size')  # Default Encoder for visual observations outputs a 1024D vector; for other embedding sizes an additional fully-connected layer is used
    ...
    parser.add_argument('--stoch-size', type=int, default=32, help='Stochastic State/latent size') # Default z_state was 32x32 in DreamerV2
    ...
    parser.add_argument('--train-seq-len', type=int, default=50, help='Sequence length for training world model') # As per official Tensorflow Implementation
    parser.add_argument('--imagine-horizon', type=int, default=15, help='Latent imagination horizon') # As per H in DreamerV2 paper
    parser.add_argument('--use-disc-model', action='store_true', help='Whether to use discount model' )
    
    # Coeffecients and constants
    parser.add_argument('--free-nats', type=float, default=3, help='Free nats') # Free nats for KL loss
    ...
    parser.add_argument('--scalar-freq', type=int, default=1e3, help='Scalar logging freq') # Amount of steps after which logging takes place
    ...
    parser.add_argument('--checkpoint-interval', type=int, default=10000, help='Checkpoint interval (episodes)') # Model Checkpoint
    ...
    logdir = os.path.join(data_path, f'{args.env}_{args.algo}_{args.exp}_{time.strftime("%d-%m-%Y-%H-%M-%S")}')
    ...
    if args.train:
        ...
        while global_step <= args.total_steps:
            ...
            if global_step % args.test_interval == 0:
                episode_rews, video_images = dreamer.evaluate(train_env, args.test_episodes)
        ...
        logs = OrderedDict()
        episode_rews, video_images = dreamer.evaluate(train_env, args.test_episodes, render=True)
